...
from src.main.service.record_data_response import RecordDataResponseStatus
...
def test_success_response(context):
    ...
    assert context.response.text == json.dumps(
        {"status": RecordDataResponseStatus.OK.value}
    )
...
def test_invalid_schema_response(context):
    ...
    """
    assert context.response.text == json.dumps(
        {"status": RecordDataResponseStatus.INVALID_SCHEMA.value}
    )
