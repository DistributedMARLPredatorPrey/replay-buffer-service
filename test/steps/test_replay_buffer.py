...
def step_impl(context):
    response: TestResponse = context.client.post("record_data/", json=context.data_record)
...
def step_impl(context):
    ...
    """
    assert context.response.text == json.dumps({"status": PostResponseStatus.INVALID_SCHEMA.value})
